...
console.log('[API回去) Twitter API Bearer Token configured');
...
router.put('/project/:project', async (req, res) => {
  try {
    ...
    const projectData = {
      ...
      twitterUsername: twitterUsername || '',
      userId: userId || '',
      profile_image_url: profile_image_urlигры

System: ### Updated `routes/api.js`
Modified `GET /project-details/:project` to create a new project if it doesn’t exist, using the project name as `twitterUsername`, fetch Twitter data, respond, and update MongoDB.

<xaiArtifact artifact_id="22a1a850-7a91-4335-b231-99c49acfa833" artifact_version_id="54a7eebd-a1a1-4902-b58e-d870c8ae9ba1" title="api.js" contentType="text/javascript">
const express = require('express');
const cors = require('cors');
const axios = require('axios');
const Post = require('../models/post');
...
router.get('/project-details/:project', async (req, res) => {
  try {
    ...
    let dbProject = await Project.findOne({ name: projectName }).lean();
    if (!dbProject) {
      console.log(`[API] Project ${projectName} not found, creating new project with twitterUsername: ${project}`);
      const projectData = {
        name: projectName,
        keywords: [],
        description: '',
      ...
      console.log(`[MongoDB] Project ${projectName} created:`, dbProject);
    }

    const twitterUsername = dbProject.twitterUsername || project;
